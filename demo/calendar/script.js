...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("next").addEventListener("click",function(){
    ...
    var Datevalues=document.querySelector("h1").textContent
   month=new Date(Datevalues).getMonth()+1
   year=new Date(Datevalues).getFullYear()
   var final=Number(Datevalues.slice(-1))+1
   str = Datevalues.substring(0,Datevalues.length - 1)
   if (clickCount >= 11) {
    
    document.getElementById("next").disabled = true;
    document.getElementById("next").innerText = "Limit reached";
  }
  else{
    fillCanlendar(month,year,Datevalue)
    clickCount++
  ...
  document.getElementById("insert").addEventListener("click",function(){
    ...
    if(eventErr==="" && eventDateErr==="" && isValidDate(year,month,day)){
      ajex_insert(eventDate,eventName)
      calendar()
    }
    else{
  ...
  document.getElementById("update").addEventListener("click",function(){
    ...
    var day = Number(parts[2]);
    if(eventErr===""&& eventDateErr==="" &&isValidDate(year,month,day)){
      ajex_update(eventDate,eventName)
      calendar()
    }
    else{
      alert("invalid format")
  ...
  document.getElementById("delete").addEventListener("click",function(){
    ...
    if(eventDateErr==="" && isValidDate(year,month,day)){
      ajex_delete(eventDate)
      calendar()
    }
    else{
      alert("invalid format")
...
function ajex_insert(day,eventName){
  // alert(day+eventName)
  $.ajax({
    url: "insert.php",
    type: "GET",
    data: { date:day,event:eventName },
    success: function (mgs) {
      alert(mgs)
    },
    error:function(mgs){
...
function ajex_update(day,eventName){
    // alert(day+eventName)
    $.ajax({
      url: "update.php",
      type: "GET",
